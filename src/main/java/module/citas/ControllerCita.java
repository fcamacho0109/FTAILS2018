...
public class ControllerCita implements Initializable {
    ...
    @FXML
    public void citaAdd() {
        // guardar todas las variables en BD !!! por el momento solo lo escribe en text area
        try {

            String paciente,mascota;
            int hora,minuto,minutoF;
            LocalDate fechaCita;
            ...
            minutoF = Integer.valueOf(text_minuto_f.getText());

            fechaCita = fecha_picker.getValue();

            text_area_nueva_cita.setVisible(true);
            text_area_nueva_cita.setText("Paciente: "+paciente+"\n"+
                    "Fecha: "+ fechaCita.toString());
            // te devuelve la string, y hay que tratarla

            crearCita(paciente,fechaCita,hora,minuto,minutoF);
    ...
    public void reAgendar() {
        // solo imprime datos una vez que se re agenda, esto imprime los nuevos datos
        try {
    ...
    /**
     * */
    public void crearCita(String paciente, LocalDate fechaCita, int hora, int minuto,int minutoF) {
        ...
        appointment.setEndLocalDateTime(LocalDate.of(fechaCita.getYear(),fechaCita.getMonth(),fechaCita.getDayOfMonth()).atTime(hora,minuto).plusMinutes(minutoF));
    ...
    public void initialize(URL location, ResourceBundle resources) {
        combox_medico.setItems(FXCollections.observableArrayList(
                //esto se va llenar con el query de doctores y su id
                "Many Rivera, 125889",
 ...
 /*agenda.appointments().addAll(
        ...
        /*agenda.newAppointmentCallbackProperty().set( (localDateTimeRange) -> { // con el muose puede mover
            return new Agenda.AppointmentImplLocal()
                    ...
                    .withAppointmentGroup(new Agenda.AppointmentGroupImpl().withStyleClass("group1"));
                    // it is better to have a map of appointment groups to get from
        });*/